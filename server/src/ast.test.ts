...
import { CallExprAST, ExprStmtAST, FunctionAST, InfixExprAST, LiteralExprAST, MissingAST, ModuleAST, NumberExprAST, ReturnStmtAST, VarDeclStmtAST, VariableExprAST } from './ast';
...
describe('ast dump', () => {
	...
	it('should dump LiteralExprAST', () => {
		const ast = new LiteralExprAST(loc, [
			new LiteralExprAST(loc, [
				new NumberExprAST(loc, 1),
				new NumberExprAST(loc, 2),
			], [2]),
			new LiteralExprAST(loc, [
				new NumberExprAST(loc, 3),
				new NumberExprAST(loc, 4),
			], [2]),
		], [2, 3]);
	...
	it('should dump VarDeclStmtAST', () => {
		const ast = new VarDeclStmtAST(loc, "foo",
			{shape: [2]},
			new LiteralExprAST(loc, [
				new NumberExprAST(loc, 1),
				new NumberExprAST(loc, 2),
	...
	it('should dump FunctionAST', () => {
		const ast = new FunctionAST(loc, "foo",
	...
	it('should dump ModuleAST', () => {
		const ast = new ModuleAST(loc, [
			new FunctionAST(loc, "foo",
				[new VariableExprAST(loc, "param1"), new VariableExprAST(loc, "param2")],
				[new ReturnStmtAST(loc, new NumberExprAST(loc, 1))]
			),
			new FunctionAST(loc, "bar",
