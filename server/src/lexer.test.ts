...
describe('tokenize', () => {
	...
	it('should tokenize comments', () => {
		const tokens = tokenizeAll(uri, '# this is a comment\n');
	...
	it('should tokenize comment without newline', () => {
		const tokens = tokenizeAll(uri, '# comment at end');
		expect(tokens).toHaveLength(2);
		expect(tokens[0].kind).toBe('COMMENT');
	...
	it('should tokenize single number', () => {
		const tokens = tokenizeAll(uri, '123');
		expect(tokens).toHaveLength(2);
	...
	it('should tokenize hexadecimal number', () => {
		const tokens = tokenizeAll(uri, '0x1a2f');
		expect(tokens).toHaveLength(2);
		expect(tokens[0].kind).toBe('NUMBER');
	...
	it('should tokenize floating point number', () => {
		const tokens = tokenizeAll(uri, '123.456');
		expect(tokens).toHaveLength(2);
		expect(tokens[0].kind).toBe('NUMBER');
	...
	it('should tokenize identifier', () => {
		const tokens = tokenizeAll(uri, 'foo_bar123');
		expect(tokens).toHaveLength(2);
	...
	it('should tokenize keywords', () => {
		...
		keywords.forEach(keyword => {
			const tokens = tokenizeAll(uri, keyword);
			expect(tokens).toHaveLength(2);
	...
	it('should tokenize punctuation', () => {
		...
		punctuations.forEach(([text, kind]) => {
			const tokens = tokenizeAll(uri, text);
			expect(tokens).toHaveLength(2);
	...
	it('should tokenize operators', () => {
		...
		operators.forEach(op => {
			const tokens = tokenizeAll(uri, op);
			expect(tokens).toHaveLength(2);
	...
	it('should tokenize simple expression', () => {
		const tokens = tokenizeAll(uri, 'var x = 42;');
	...
	it('should tokenize arithmetic expression', () => {
		const tokens = tokenizeAll(uri, 'a + b * 3');
		expect(tokens).toHaveLength(6);
	...
	it('should tokenize function definition', () => {
		const tokens = tokenizeAll(uri, 'def foo(x, y) { return x + y; }');
...
var y = 2;
`;
		const tokens = tokenizeAll(uri, source);
	...
	describe('location information', () => {
		it('should provide correct location for single token', () => {
			const tokens = tokenizeAll(uri, 'foo');
		...
		it('should provide correct location for number literals', () => {
			// Integer
			const tokens1 = tokenizeAll(uri, '123');
			...
			// Float
			const tokens2 = tokenizeAll(uri, '123.456');
			...
			// Hex
			const tokens3 = tokenizeAll(uri, '0x1a2f');
		...
		it('should provide correct location with leading whitespace', () => {
			const tokens = tokenizeAll(uri, '   var x = 1;');
